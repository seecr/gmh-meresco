...
integrationTempdir = '/data/meresco/integration'
reactor = Reactor()
...
class IntegrationTest(CQ2TestCase):
    ...
    def testIndex(self):
        ...
        self.assertSruQuery(1, 'dais="info:eu-repo/dai/nl/071792279"')

    def testDrilldown(self):
        result = self.doDrilldown('unqualifiedterms="knaw_mir"', 'genre')
        navigator = result.extraResponseData.drilldown.term_drilldown.navigator
        
        self.assertEquals(1, len(navigator), result.xml())
        self.assertEquals('genre', str(navigator.name))
        self.assertEquals(3, len(navigator.item)) # select distinct drillD. fieldname
        itemValues = [(item.count, str(item)) for item in navigator.item]
        self.assertEquals([(1, 'article'), (1, 'book'), (2, 'doctoralthesis')], itemValues)

    def testRSS(self):
        body = self._doQuery({'query': 'knaw_mir', 'maximumRecords': '20', 'startRecord' : '1'}, path="/rss") 
        items = [(str(item.title), str(item.description), str(item.link).split('RecordID', 1)[1]) for item in body.rss.channel.item]
        #print 'BODY:', body.xml()
        #for item in items:
            #print 'ITEM:', item
    ...
    def testOaiListRecords(self):
        header, body = getRequest(reactor, port, '/oai', {'verb': 'ListRecords', 'metadataPrefix': 'knaw_short'})
        #print 'HEADER', header
        #print 'BODY', body.xml()
        self.assertEquals('HTTP/1.0 200 Ok\r\nContent-Type: text/xml; charset=utf-8', header)
    ...
    def testDeleteRecord(self):
        ...
        header, body = getRequest(reactor, port, '/oai', {'verb': 'ListRecords', 'metadataPrefix': 'knaw_short'})
        #print 'ListRecords: ' , body.OAI_PMH.ListRecords.xml()
        self.assertEquals('deleted', body.OAI_PMH.ListRecords.record[4].header.status)
