CREATE DATABASE  IF NOT EXISTS `tst_nbnresolver` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `tst_nbnresolver`;
...
DELIMITER ;;
CREATE DEFINER=`tst_nbnresolver`@`localhost` PROCEDURE `deleteNbnLocationsByRegistrant`(IN nbn_value VARCHAR(510), IN registrantid INT(11))
BEGIN

DECLARE identifierid BIGINT(20);
DECLARE failover BOOLEAN;

START TRANSACTION;

	SELECT I.identifier_id INTO identifierid FROM identifier I WHERE I.identifier_value = nbn_value;
    SELECT R.isLTP INTO failover FROM registrant R WHERE R.registrant_id = registrantid;

-- Deze delete alle record uit IL en corresponderende records uit LR. Hij laat Locatie's en NBN staan in de betreffende tabellen.
	DELETE IL, LR FROM identifier_location IL INNER JOIN location_registrant LR ON IL.location_id = LR.location_id WHERE LR.registrant_id = registrantid AND IL.identifier_id = identifierid AND IL.isFailover = failover;

-- Cleanup locations table, but do not bother if some keys still exist in reference tables:
	DELETE IGNORE l FROM location l  LEFT JOIN identifier_location IL ON IL.location_id = l.location_id WHERE IL.location_id IS NULL;
...
-- Dumping routines for database 'tst_nbnresolver'
--
/*!50003 DROP PROCEDURE IF EXISTS `insertNbnLocation` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
CREATE DEFINER=`tst_nbnresolver`@`localhost` PROCEDURE `insertNbnLocation`(IN nbn_value VARCHAR(510), IN nbn_location VARCHAR(1022), IN registrant_id INT(11) )
BEGIN
...
START TRANSACTION;
    ...
    SELECT R.isLTP INTO failover FROM registrant R WHERE R.registrant_id = registrant_id;
	...
	IF (failover) THEN
		SET @location = loc_locationid, @identifier = loc_identifierid, @last_update=NOW(), @failover=failover;
