CREATE DATABASE  IF NOT EXISTS `gmhtest` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `gmhtest`;
...
DELIMITER ;;
CREATE DEFINER=`gmhtest`@`localhost` PROCEDURE `addNbnLocation`(IN nbn_value VARCHAR(510), IN nbn_location VARCHAR(1022), IN registrant_id INT(11) , IN failover BOOLEAN)
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_general_ci */ ;
DELIMITER ;;
CREATE DEFINER=`gmhtest`@`localhost` PROCEDURE `deleteNbnLocationsByRegistrantId`(IN nbn_value VARCHAR(510), IN registrantid INT(11), IN failover BOOLEAN)
BEGIN
