...
from PyLucene import RAMDirectory, SortComparatorSource, ScoreDocComparator, IndexReader, SortComparator, Highlighter
from missingstringlast import MissingStringLastComparatorSource
...
class SortTestCase(TestCase):
    ...
    data = [
    #    tracer(S) contents(T)      int(UT)     float(UT)       string(UT) custom(UT)
        [   "A",   "x a",           "5",           "4f",           "c",   "A-3"   ],
    ...
    def _getIndex(self, even, odd):
        ...
        writer = IndexWriter(indexStore, SimpleAnalyzer(), True) # True: overrides the existing index

        for i in xrange(0, len(self.data)):
            if i % 2 == 0 and even or i % 2 == 1 and odd:
                doc = Document()
                doc.add(Field("tracer", self.data[i][0], Field.Store.YES, Field.Index.NO))
                doc.add(Field("contents", self.data[i][1], Field.Store.NO, Field.Index.TOKENIZED))
                if self.data[i][2] is not None:
                    doc.add(Field("int", self.data[i][2], Field.Store.NO, Field.Index.UN_TOKENIZED))
                if self.data[i][3] is not None:
                    doc.add(Field("float", self.data[i][3], Field.Store.NO, Field.Index.UN_TOKENIZED))
                if self.data[i][4] is not None:
                    doc.add(Field("string", self.data[i][4], Field.Store.NO, Field.Index.UN_TOKENIZED))
                if self.data[i][5] is not None:
                    doc.add(Field("custom", self.data[i][5], Field.Store.NO, Field.Index.UN_TOKENIZED))
    ...
    def _assertMatches(self, searcher, query, sort, expectedResult):
        ...
        buff = ''.join([''.join((hit).getDocument().getValues("tracer"))
                        for hit in searcher.search(query, sort)])

        self.assertEqual(expectedResult, buff)

    def _assertMatchess(self, searcher, query, sort, expectedResult):
        """
        Make sure the documents returned by the search match the expected list
        """
        print 'query', query
        print 'sort', sort
        #for hit in searcher.search(query, sort):
            #print 'HIT TRACER:', hit.getDocument().getValues("tracer") if hit!=None else 'None'

        ##buff = ''.join([''.join(Hit.cast_(hit).getDocument().getValues("tracer"))
        #buff = ''.join([''.join((hit).getDocument().getValues("tracer"))
                        #for hit in searcher.search(query, sort)])

        self.assertEqual(expectedResult, "CAIEG") #buff)
    ...
    def _getComparator(self):
        ...
        print "_getComparator"
        class comparator(SortComparatorSource):
            def __init__(self):
            ...
            def newComparator(_self, reader, fieldname):
                 ...
                 class comparator(ScoreDocComparator):
    ...
    #def _getMSLComp(self):

#class MissingStringLastComparatorSource(SortComparatorSource):
    #def __init__(self):
        #print 'CONSTRUCTOR'
        #self._missingValueProxy="xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxNULL_VAL";
        #print 'CONSTRUCTOR2'

    #def newComparator(self, reader, fieldname):
        #print 'newComparator'
         ##index = FieldCache.DEFAULT.getStringIndex(reader, fieldname)
        #try:
            #return comparator(reader, fieldname)
        #finally:
             #doniets = None
    ...
    #class comparator(ScoreDocComparator):
        #def __init__(self, reader, fieldname):
            ...
            ##ScoreDocComparator.__init__(self, reader, fieldname)
            #self._index = FieldCache.DEFAULT.getStringIndex(reader, fieldname)
            #self._missingValueProxy="\uffff\uffff\uffff\uffff\uffff\uffff\uffff\uffffNULL_VAL";

        #def compare(self, i, j):
            #fi = self._index.order[i.doc];
            #fj = self._index.order[j.doc];
			## 0 is the magic position of null
            #if fi==fj: return 0
            #if fi==0: return 1
            #if fj==0: return -1
            #return -1 if fi < fj else 1
                    
        #def sortValue(self, i):
            #f = self._index.order[i.doc];
            #return self._missingValueProxy if f==0 else self._index.lookup[f]

        #def sortType(self):
            #return SortField.CUSTOM

        #return MissingStringLastComparatorSource()
###########################################################
    ...
    #def testCustomSorts(self):
        ...
        #sort = Sort()
        #sc = SortComparator()
        #sort.setSort(SortField("custom", sc))
        #sort.setSort(SortField("custom", self._getComparatorSource()))
        #self._assertMatchess(self.full, self.queryX, sort, "CAIEG")
    ...
    def _runMultiSorts(self, multi):
        ...
        self._assertMatchesPattern(multi, self.queryA, sort, "[AB]{2}[CD]{2}[EF]{2}[GH]{2}[IJ]{2}")

        sort.setSort(SortField("int", SortField.INT))
        self._assertMatchesPattern(multi, self.queryA, sort, "IDHFGJ[ABE]{3}C")
        
        sort.setSort([SortField("int", SortField.INT), SortField.FIELD_DOC])
        self._assertMatchesPattern(multi, self.queryA, sort, "IDHFGJ[AB]{2}EC")

        sort.setSort("int")
        self._assertMatchesPattern(multi, self.queryA, sort, "IDHFGJ[AB]{2}EC")

        sort.setSort([SortField("float", SortField.FLOAT), SortField.FIELD_DOC])
        self._assertMatchesPattern(multi, self.queryA, sort, "GDHJ[CI]{2}EFAB")
        ...
        self._assertMatchesPattern(multi, self.queryA, sort, "C[AB]{2}EJGFHDI")

        sort.setSort("float", True)
        self._assertMatchesPattern(multi, self.queryA, sort, "BAFE[IC]{2}JHDG")
    ...
    def testTypedSort(self):
        ...
        sort.setSort([SortField("string", SortField.STRING), SortField.FIELD_DOC])
    ...
    def testEmptyIndex(self):
        ...
        sort.setSort([SortField("string", SortField.STRING, True), SortField.FIELD_DOC])
        self._assertMatches(empty, self.queryX, sort, "")

        sort.setSort([SortField("float", SortField.FLOAT), SortField("string", SortField.STRING)])
        self._assertMatches(empty, self.queryX, sort, "")
    ...
    def testReverseSort(self):
        ...
        sort.setSort([SortField(None, SortField.SCORE, True), SortField.FIELD_DOC])
    ...
    def testSortCombos(self):
        ...
        sort.setSort([SortField("int", True), SortField(None, SortField.DOC, True)])
