...
#1:     Get persistentIdentifier:
        ...
        if pidlist:
            ...
            raise ValidateException(formatException("Mandatory persistent identifier (urn:nbn) not found in top level Item.", self._identifier))
...
#2:     Get toplevel modificationDate: self._validateISO8601()
        ...
        if tl_modified and not self._validateISO8601(tl_modified[0]):
            raise ValidateException(formatException("Mandatory date modified in toplevelItem not a valid ISO8601 date: " + tl_modified[0], self._identifier))
        elif not tl_modified:
            raise ValidateException(formatException("Mandatory date modified in toplevelItem missing.", self._identifier))     
...
#4:     Get PidResourceLocation:
        ...
        if pidlocation == '':
            raise ValidateException(formatException("Mandatory resource location not found in top level Item.", self._identifier))
    ...
    #TODO: This always normalizes to rdf namespace, without warning/message!
        ...
        if descriptiveMetadataItem:
            mods_template = """<didl:Item>
                                ...
                                </didl:Item>""" % (self._getDateModifiedDescriptor(descriptiveMetadataItem[0]), self._getMODS(descriptiveMetadataItem[0]))
        else:
            raise ValidateException(formatException("Mandatory descriptiveMetadata item element not found in DIDL part.", self._identifier))
    ...
    def _getMODS(self, lxmlNode):    
        ...
        if mods:
            ...
            raise ValidateException(formatException("Mandatory MODS in descriptiveMetadata element not found in DIDL record.", self._identifier))
    ...
    def _getObjectfiles(self, lxmlNode):              
        ...
        #8:Check for MANDATORY resources and mimetypes:
            ...
            if resources != '':
                ...
                raise ValidateException(formatException("Mandatory Resource not found in ObjectFile Item.", self._identifier))            
...
#                of_container = ''
    ...
    def _checkURNFormat(self, pid):
        ...
        m = p.match(pid)
        if not m:
            raise ValidateException(formatException("Invalid format for mandatory persistent identifier (urn:nbn) in top level Item: " + pid, self._identifier))
        return True     
