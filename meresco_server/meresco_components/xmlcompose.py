...
class XmlCompose(Observable):
    ...
    def getRecord(self, identifier):
        ...
        for tagName, values in self._fieldMapping.items():
            ...
            result = xml.xpath(xPathExpression, namespaces=self._nsMap) # , smart_strings=False
            
            # http://lxml.de/xpathxslt.html#xpath-return-values
            # xpath() function may return different return-types!
            if len(result) > 0:
                if type(result[0]).__name__ == 'tuple': # Namespaces. We can return them as tuple: (prefix, URI).
                    data[tagName] = str(result[0][1])
                elif type(result[0]).__name__ == 'str':
                    data[tagName] = ''.join(result)
                else:
