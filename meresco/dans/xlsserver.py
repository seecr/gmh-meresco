...
# Copyright (C) 2012-2013, 2015, 2017, 2025 Seecr (Seek You Too B.V.) https://seecr.nl
...
class File(object):
    ...
    def stream(self):
        ...
        while data:
            ...
            data = fp.read(1024)
        fp.close()
        if isdir(os.path.dirname(self._filename)) and self._filename.startswith(
            "/tmp/"
        ):
    ...
    def _createXLS(self, repositoryId):
        ...
        if config is not None and invalidrecordIds:
            ...
            for idx, recId in enumerate(invalidrecordIds):
                ...
                if len(recordId) > max_col:
                    max_col = len(recordId)
                ...
                basisurl = (
                    "%(baseurl)s?verb=GetRecord&identifier=%(oai_id)s&metadataPrefix=%(mdpf)s"
                    % {
