...
class Resolver(Observable):
    ...
    def add(self, identifier, partname, lxmlNode, **kwargs):
        ...
        nbnlocation = f_normdoc.xpath(
            "//didl:DIDL/didl:Item/didl:Component/didl:Resource/@ref",
            namespaces=self._nsMap,
        )
        urnnbn = f_normdoc.xpath(
            "//didl:DIDL/didl:Item/didl:Descriptor/didl:Statement/dii:Identifier/text()",
            namespaces=self._nsMap,
        )[0]
        repositorygroupid = f_meta.xpath(
            "//meta:meta/meta:repository/meta:repositoryGroupId/text()",
            namespaces=self._nsMap,
        )[0]

        yield self.do.addNbnToDB(
            identifier, locations=nbnlocation, urnnbn=urnnbn, rgid=repositorygroupid
        )
