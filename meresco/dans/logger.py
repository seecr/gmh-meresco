...
class Logger(Observable):
    ...
    def logMsg(self, identifier, logmsg, prefix=None):
        if self._enabled:
            ...
            logger = getRssLogger(
                identifier.split(":", 1)[0], self._logfileDir
            )  # repositoryId, logfileDir
    ...
    def _getMetaPartStuff(self, uploadId):
        ...
        if meta is not None:
            baseu = meta.xpath(
                ...
                namespaces={"meta": "http://meresco.org/namespace/harvester/meta"},
            )
            prefix = meta.xpath(
                "//meta:repository/meta:metadataPrefix/text()",
                namespaces={"meta": "http://meresco.org/namespace/harvester/meta"},
            )
    ...
    def _getUniqueLogLines(self, repositoryId, maxlines):
        ...
        while len(log_dict) < maxlines and bckpcnt <= BACKUPCOUNT:
            ...
            file_name = (
                repositoryId if bckpcnt == 0 else (repositoryId + "." + str(bckpcnt))
            )
            ...
            if isfile(join(self._logfileDir, file_name)):
                ...
                with open(join(self._logfileDir, file_name), "r") as file:
                    ...
                    for line in file:
                        log_dict[line.split(" ", 2)[2]] = (
                            line  # log_dict [line.split(' ', 3)[3]] = line
                        )
...
def getRssLogger(repositoryId, logfileDir):
    ...
    rfh = logging.handlers.RotatingFileHandler(
        (LOG_FILENAME), maxBytes=MAXLOGSIZE, backupCount=BACKUPCOUNT
    )
